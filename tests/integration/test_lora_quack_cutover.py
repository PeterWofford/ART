...
def test_lora_grouped_cutover_rejects_rank_12() -> None:
    if not torch.cuda.is_available():
        pytest.skip("CUDA is required for the LoRA QuACK cutover test.")

    device = torch.device("cuda:0")
    counts = torch.tensor([32, 0, 16, 24], dtype=torch.int64)
    x = torch.randn(int(counts.sum().item()), 64, device=device, dtype=torch.bfloat16)
    ...
    with pytest.raises(ValueError, match="rank < 8 or a multiple of 8"):
        _ = lora(x, tokens_per_expert=counts)

    gate_a_t = torch.randn(4, 64, 12, device=device, dtype=torch.bfloat16)
    ...
    up_b_t = torch.randn(4, 12, 64, device=device, dtype=torch.bfloat16)
    with pytest.raises(ValueError, match="rank < 8 or a multiple of 8"):
        _ = quack_grouped_lora_dual(
            x,
            gate_a_t,
            gate_b_t,
            up_a_t,

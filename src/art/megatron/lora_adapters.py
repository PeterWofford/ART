...
_LOGGED_LORA_MISSES: set[str] = set()
...
class LoRA(torch.nn.Module):
    def __init__(
        ...
        lora_dtype = dtype if dtype in (torch.float16, torch.bfloat16) else torch.bfloat16
        if _MEGATRON_DEBUG and lora_dtype != dtype:
            logger.warning(
                "Promoting LoRA dtype from %s to %s for optimizer/main_grad compatibility.",
                dtype,
                lora_dtype,
            )
        self.A_T = torch.nn.Parameter(
            torch.zeros(
                num_local_experts, in_features, rank, dtype=lora_dtype, device=device
            ).squeeze(0)
        )
        self.B_T = torch.nn.Parameter(
            torch.zeros(
                num_local_experts, rank, out_features, dtype=lora_dtype, device=device
            ).squeeze(0)
        )
    ...
    def load_lora(self, adapter_model: dict[str, torch.Tensor]) -> None:
        try:
            ...
            if self.adapter_model_prefix not in _LOGGED_LORA_MISSES:
                _LOGGED_LORA_MISSES.add(self.adapter_model_prefix)
                missing_key = exc.args[0] if exc.args else "<unknown>"
                sample_keys = sorted(adapter_model.keys())[:8]
                logger.warning(
                    "LoRA key miss for prefix=%s missing_key=%s tensors=%d sample_keys=%s",
                    self.adapter_model_prefix,
                    missing_key,
                    len(adapter_model),
                    sample_keys,
                )
            elif _MEGATRON_DEBUG:
                logger.debug(
                    "Repeated LoRA key miss for prefix=%s",
                    self.adapter_model_prefix,
                )
    ...
    def forward(
        ...
        input_dtype = x.dtype
        compute_x = x
        if compute_x.dtype != self.A_T.dtype:
            compute_x = compute_x.to(self.A_T.dtype)
        if tokens_per_expert is not None:
            ...
            # grouped_gemm kernels require bf16 inputs; enforce here for
            # unfused-attention training paths that promote activations.
            gmm_x = (
                compute_x
                if compute_x.dtype == torch.bfloat16
                else compute_x.to(torch.bfloat16)
            )
            gmm_a = (
                self.A_T if self.A_T.dtype == torch.bfloat16 else self.A_T.to(torch.bfloat16)
            )
            gmm_b = (
                self.B_T if self.B_T.dtype == torch.bfloat16 else self.B_T.to(torch.bfloat16)
            )
            tmp = grouped_gemm_util.ops.gmm(gmm_x, gmm_a, bsz, trans_b=False)  # type: ignore[attr-defined]
            out = grouped_gemm_util.ops.gmm(tmp, gmm_b, bsz, trans_b=False)  # type: ignore[attr-defined]
            out = out * self.scale
        else:
            out = ((compute_x @ self.A_T) @ self.B_T) * self.scale

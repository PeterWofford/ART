...
def get_compute_loss_fn(trainer: "GRPOTrainer") -> Callable[..., torch.Tensor]:
    def compute_loss(
        ...
        trainer._metrics["train"]["policy_loss"].append(loss.policy_loss.item())
        if loss.entropy is not None:
            trainer._metrics["train"]["entropy"].append(loss.entropy.item())
        if config.beta > 0.0:
            trainer._metrics["train"]["kl_div"].append(loss.kl.item())
        if loss.kl_policy_ref is not None:
            trainer._metrics["train"]["kl_policy_ref"].append(loss.kl_policy_ref.item())
        return loss.policy_loss + config.beta * loss.kl
